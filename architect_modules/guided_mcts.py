...
class NeuralMCTSNode(MCTSNode):
    ...
    def __init__(self, architecture: NeuralArchitecture, policy_value: Dict = None,
        ...
        # Cache for valid actions. Invalidate when phase changes.
        self._actions_cache_phase = None
...
class NeuralMCTS(MCTS):
    ...
    def search(self, initial_architecture: NeuralArchitecture, iterations: int = 100,
               temperature: float = 1.0, reuse_root: NeuralMCTSNode = None,
               evolutionary_cycle: 'EvolutionaryCycle' = None) -> NeuralMCTSNode:
        ...
        Args:
            ...
            evolutionary_cycle (EvolutionaryCycle): The current evolutionary
                cycle from the trainer, used to synchronize state.

        Returns:
            NeuralMCTSNode: The node corresponding to the best action found.
        """
        # Synchronize MCTS's internal cycle with the trainer's cycle
        if evolutionary_cycle is not None:
            self.current_cycle = evolutionary_cycle.copy()
        ...
        if reuse_root is not None:
            ...
            # CRITICAL: Clear children from previous search. The reused root
            # represents a new state (after the selected action was taken), so
            # its children, which represent subsequent actions, are now invalid
            # and must be regenerated by the new search.
            root.children.clear()
        else:
            root = NeuralMCTSNode(initial_architecture)

        # Populate root node with current valid action
        print("Episode phase", evolutionary_cycle.get_phase_value())
        print("Current MCTS cycle phase", self.current_cycle.get_phase_value())
        ...
        try:
            ...
            try:
                print("Selecting final action under episode phase", orig_cycle.get_phase_value())
                ...
                raise RuntimeError("Failed to retrieve valid actions for final action selection under episode-level evolutionary cycle.")
            ...
            if episode_legal_children:
                ...
                raise RuntimeError("No valid actions available under episode-level evolutionary cycle during final action selection.")
    ...
    def _expand(self, node: NeuralMCTSNode) -> tuple:
        ...
        t0 = time.time()
        # Stale cache check: if the node's action cache belongs to a
        # different phase, invalidate it.
        if (node._actions_cache_phase != current_phase):
            node._unexpanded_actions_dict = None
        ...
        if (node._unexpanded_actions_dict is None):
            ...
            # Track which phase the valid-actions cache belongs to
            node._actions_cache_phase = current_phase
